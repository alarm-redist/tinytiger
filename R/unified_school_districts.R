...
tt_unified_school_districts <- function(state, year = 2024) {
  ...
  shp <- lapply(state, function(st) {
    zip_url <- glue::glue("{base_url(year)}/UNSD{url_adj}/tl_{year}_{st}_unsd{dec_yr}.zip")
    target <- glue::glue("tl_{year}_{st}_unsd{dec_yr}.shp")
